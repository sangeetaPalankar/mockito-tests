...
@Test
void doctorCount() throws Exception {
	when(physicianAvailabilityService.countDoctors()).thenReturn(2L);
	this.mockMvc.perform(get("http://localhost:9007/api/v1/doctorCount"))
...
@Test
void postDoctors() throws Exception, Throwable {
	doNothing().when(physicianAvailabilityService).postDoctors();

	this.mockMvc.perform(get("http://localhost:9007/api/v1/addDoctors"))
