...
//	}
	...
	void getPreviousVisitDetailsByPatientIdforhistory() {
		...
		assertEquals(v1,patientHealthService.findById(111));		
	}
	...
	void findTestByVisitIdTest() {
		...
		list.add(t1);
		when(testrepo.findTestByVisitId(101)).thenReturn(list);
		when(patientHealthService.findTestByVisitId(101)).thenReturn(list);
		assertEquals(t1,patientHealthService.findTestByVisitId(101));
	...
	public void findVisitDetailsByVisitId() {
		...
		assertEquals(list,patientHealthService.findById(1001));
...
//		assertThat(savedEmployee).isNotNull();
	...
	public void deleteTest() {
		Tests t1 = new Tests(101, "Blood", "Normal", "normal", 10001);
		when(testrepo.findById(anyInt())).thenReturn(Optional.of(t1));
		doNothing().when(testrepo).deleteById(anyInt());
//		assertEquals(patientHealthService.deleteTest(101),true);	
	}
	
	@Test
	void deletePrescription() {
		Prescription p1 = new Prescription(10001, "dolo", "1-0-1", "after lunch",101);
		when(prescriptionrepo.findById(anyInt())).thenReturn(Optional.of(p1));
		doNothing().when(prescriptionrepo).deleteById(anyInt());
		assertEquals(patientHealthService.deletePrescription(10001),true);	
	...
	void deleteNurse() {
		NurseInfo n1 = new NurseInfo("nurse1@gmail.com","Riya", "Shetty");
		when(nurseRepo.findById(anyString())).thenReturn(Optional.of(n1));
		doNothing().when(nurseRepo).deleteById(anyString());
//		assertEquals(patientHealthService.deleteNurse("nurse1@gmail.com"),true);
...
//		assertEquals(patientHealthService.deleteAdmin("admin1@gmail.com"),true);
	...
	void updateForPrescriptionByPrescriptionId() {
		...
		assertEquals(patientHealthService.updateForPrescriptionByPrescriptionId(id,p1),true);
	...
	void updateForTestByTestId() {
		...
		when(testrepo.findAll()).thenReturn(list);
		when(testrepo.save(t1)).thenReturn(t1);	
		assertEquals(patientHealthService.updateForTestByTestId(testid,t1),t1);
