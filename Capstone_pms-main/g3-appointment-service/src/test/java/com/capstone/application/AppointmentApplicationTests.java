...
class AppointmentApplicationTests {
	...
	@Test
	void findPreviousAppointmentforNurseDoctor() throws Exception {	
		when(appointmentService.findPreviousAppointmentforNurseDoctor(anyInt())).thenReturn(aP1);
		
		this.mockMvc.perform(get("http://localhost:9003/api/v1/appointment/{patientId}/previous/nurse",1))
		.andExpect(status().isOk());
	}
